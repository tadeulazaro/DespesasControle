...
//app.set('view engine', 'despesas.html');
...
function apagarDespesa(despesaId) {
  fetch(`/apagarDespesa/${despesaId}`, {
    method: 'DELETE'
  })
  .then(response => {
    if (response.ok) {
      // A despesa foi excluída com sucesso
      // Remova a linha da tabela correspondente no DOM
      const despesaRow = document.querySelector(`tr[data-despesa-id="${despesaId}"]`);
      if (despesaRow) {
        despesaRow.remove();
      }
    } else {
      // Lida com erros se a exclusão falhar
...
app.delete('/apagarDespesa/:despesaId', async (req, res) => {
  const despesaId = req.params.despesaId;

  try {
    const despesaRemovida = await Despesa.findByIdAndRemove(despesaId);

    if (despesaRemovida) {
      return res.redirect('/despesas');
      res.status(200).json({ message: 'Despesa excluída com sucesso.' });
    } else {
      // Se não encontrou uma despesa com o ID especificado, retorne um erro 404
